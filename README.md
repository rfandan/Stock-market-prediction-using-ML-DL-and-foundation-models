...
## 🚀 Project Overview
This project implements a modular, 7-stage research pipeline to predict market returns. Instead of just training one model, we benchmark a wide variety of architectures to determine which "AI DNA" best handles the noise of financial time-series data.

## 🛠 Project Roadmap
The project is divided into specialized stages. It is recommended to run them in order:
...
3.  **Run the Pipeline**:
    Open the `notebook/` folder. **Crucial First Step**: Run `data_download_colab.ipynb` first to fetch historical data, then begin with `Stage1.ipynb`.
